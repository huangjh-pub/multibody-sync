...
at::Tensor ball_query_var_radius(at::Tensor new_xyz, at::Tensor xyz, at::Tensor radius,
                                 const int nsample) {
  CHECK_CONTIGUOUS(new_xyz);
  CHECK_CONTIGUOUS(xyz);
  ...
  if (new_xyz.type().is_cuda()) {
    query_ball_point_var_radius_kernel_wrapper(xyz.size(0), xyz.size(1), new_xyz.size(1),
